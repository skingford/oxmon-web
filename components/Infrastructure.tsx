...
const Infrastructure: React.FC<InfrastructureProps> = ({ agents, onShowToast }) => {
    ...
    const handleSimulateFailure = async () => {
        ...
        try {
            ...
            const prompt = `Simulate a critical node failure for "${selectedNode.name}" (${selectedNode.ip}). Provide a cascading risk profile for the global Oxmon mesh in 3 high-density technical bullet points.`;
            const response = await ai.models.generateContent({ model: 'gemini-3-flash-preview', contents: prompt });
            setFailureAnalysis(response.text || 'Simulation link timeout.');

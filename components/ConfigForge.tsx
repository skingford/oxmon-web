  ...
  const handleGenerate = async (overriddenPrompt?: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: `Act as a senior Infrastructure Architect. Generate a production-ready, high-fidelity configuration for: "${finalPrompt}". Return ONLY the code block. Include expert comments on performance.`
      });
      let code = response.text || 'Synthesis failed.';
      
      if (isHardeningActive) {
          onShowToast('Initiating neural hardening sequence...', 'info');
          ...
          if (parts.length >= 3) {
              ...
              setHardeningReport(parts[0] + (parts[2] || ''));
          } else {
              code = result;
          }
      }
      ...
      setHistory(prev => [{ id: Math.random().toString(36).substr(2, 9), title: finalPrompt.substring(0, 30) + '...', code, date: new Date().toLocaleTimeString() }, ...prev].slice(0, 10));

  ...
  const executeCommand = async (cmd: string, agent: Agent) => {
    ...
    try {
        ...
        const response = await ai.models.generateContent({ 
            ...
            contents: `Terminal simulation for "${agent.name}" on ${agent.os || 'Linux'}. Executing: "${cmd}". Provide professional stdout.` 
        });
        setTerminalOutput(prev => [...prev, { type: 'resp', text: response.text || 'Success.' }]);
    } catch (err) {

...
import { TeamMember, AppPreferences } from '../types';
...
const Settings: React.FC<SettingsProps> = ({ 
  ...
  const handleNeuralAudit = async () => {
    ...
    try {
      ...
      Team: ${JSON.stringify(teamMembers.map(m => ({ role: m.role, status: m.status })))}
      MFA Enabled: ${preferences.secure2FA}
      Workspace: ${preferences.workspaceName}
      Provide a professional security risk assessment in 3 bullet points.`;
      const response = await ai.models.generateContent({ model: 'gemini-3-pro-preview', contents: prompt });
      setAuditReport(response.text || 'Audit concluded with no identified vulnerabilities.');

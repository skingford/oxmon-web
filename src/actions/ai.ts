...
export async function interpretCommand(input: string): Promise<{ action: string; target: string; message: string }> {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `Interpret intent: "${input}". Options: dashboard, agents, metrics, infrastructure, domains, alerts, logs, tools, settings. Return JSON: { "action": "navigate", "target": "view_name", "message": "feedback" }`,
      config: {

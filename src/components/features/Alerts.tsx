    ...
    <div className="space-y-10 animate-fade-in pb-20">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <div className="h-40 relative z-10">
                    <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={ACTIVITY_DATA}>
                            ...
                            <Area type="monotone" dataKey="val" stroke="#FF3B30" strokeWidth={4} fillOpacity={1} fill="url(#colorPulse)" isAnimationActive={false} />
                        </AreaChart>
                    </ResponsiveContainer>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6">
                {filteredAlerts.length > 0 ? filteredAlerts.map((alert) => (
                    ...
                    >
                        <div className={`w-16 h-16 rounded-[1.75rem] shrink-0 flex items-center justify-center border-2 transition-all ${
                            alert.severity === 'Critical' ? 'bg-red-50 text-danger border-red-100 shadow-2xl shadow-danger/10' :
                            alert.severity === 'Warning' ? 'bg-orange-50 text-warning border-orange-100' : 'bg-blue-50 text-primary border-blue-100'
                        }`}>
                            <span className={`material-symbols-outlined text-[32px] ${alert.severity === 'Critical' ? 'animate-pulse filled' : ''}`}>{alert.severity === 'Critical' ? 'emergency' : 'warning'}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-4 mb-2">
                                <span className="text-[11px] font-black uppercase tracking-[0.25em] text-secondary font-mono opacity-60 truncate">{alert.source}</span>
                                <span className="text-[11px] text-secondary/30 tracking-widest">•</span>
                                <span className="text-[11px] font-black uppercase tracking-widest text-secondary/60">{alert.time}</span>
                            </div>
                            <h4 className="text-lg font-black text-text-main truncate group-hover:text-primary transition-colors tracking-tight leading-none">{alert.message}</h4>
                        </div>
                        <div className="flex items-center gap-4">
      ...
      {detailAlert && (
          <div className="fixed inset-0 z-[120] flex justify-end">
              <div className="absolute inset-0 bg-[#020617]/80 backdrop-blur-md animate-fade-in" onClick={() => { setDetailAlert(null); setDiagnosticResult(null); }}></div>
              <div className="relative w-full max-w-2xl bg-white h-full shadow-3xl border-l border-border p-16 animate-slide-in-right flex flex-col overflow-hidden">
                  <div className="flex justify-between items-start mb-16">
                      ...
                      <button onClick={() => { setDetailAlert(null); setDiagnosticResult(null); }} className="w-12 h-12 flex items-center justify-center text-secondary hover:text-text-main bg-gray-50 rounded-2xl transition-all"><span className="material-symbols-outlined">close</span></button>
                  </div>
                          ...
                          {diagnosticResult ? (
                              <div className="space-y-10 animate-fade-in-up">
                                  <div className="bg-[#0F172A] border border-white/5 rounded-[3rem] p-12 relative overflow-hidden shadow-3xl group">
                                      <div className="absolute top-0 right-0 p-12 opacity-5 pointer-events-none group-hover:scale-110 transition-transform"><span className="material-symbols-outlined text-[120px] text-white">neurology</span></div>

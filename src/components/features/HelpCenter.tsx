...
function renderInsightIcon(icon: InsightItem['icon']) {
  if (icon === 'speed') return <Gauge className="text-[28px]" />
  if (icon === 'shield_with_heart')
    return <ShieldPlus className="text-[28px]" />
  if (icon === 'settings_backup_restore')
    return <RefreshCw className="text-[28px]" />
  return <Brain className="text-[28px]" />
}
    ...
    <div className="space-y-16 animate-fade-in max-w-6xl mx-auto h-full flex flex-col pb-16">
      <div className="text-center space-y-6">
        <div className="w-24 h-24 bg-primary/10 text-primary rounded-[2.5rem] flex items-center justify-center mx-auto shadow-soft ring-[12px] ring-primary/5">
          <BrainCircuit className="text-5xl" />
        </div>
        <h2 className="text-5xl font-black text-text-main tracking-tighter uppercase">
          {tr('Knowledge Hub')}
        </h2>
        <p className="text-secondary max-w-2xl mx-auto leading-[2] text-base font-medium">
          {tr(
            'Neural advisor for global infrastructure patterns, enterprise DevOps best practices, and Oxmon technical documentation.',
          )}
        </p>
      </div>
      ...
      <div className="relative group rounded-[4rem] border border-[#E5E5EA] bg-white p-4 shadow-2xl transition-all focus-within:ring-[14px] focus-within:ring-primary/5">
        <form onSubmit={handleSearch} className="flex items-center">
          <Search className="ml-8 text-gray-300 group-focus-within:text-primary transition-colors text-[32px]" />
          <Input
            type="text"
            value={query}
        ...
        {!answer && !isSearching && (
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 animate-fade-in">
            <div className="lg:col-span-4">
              <h4 className="text-[11px] font-black text-secondary uppercase tracking-[0.4em] mb-8">
                {tr('Sentinel Trending Insights')}
              </h4>
            </div>
            {TRENDING_INSIGHTS.map((insight) => (
              ...
              >
                ...
                <div>
                  <p className="text-base font-black text-text-main uppercase tracking-tight mb-2 group-hover:text-primary transition-colors">
                    {tr(insight.title)}
                  </p>
                  <p className="text-[11px] text-secondary font-bold leading-relaxed opacity-60">
                    {tr(insight.sub)}
                  </p>
                </div>
              </Button>
        ...
        {isSearching && (
          <div className="space-y-16 animate-fade-in-up">
            <div className="bg-white rounded-[4rem] border border-[#E5E5EA] p-20 shadow-soft animate-pulse flex flex-col items-center justify-center space-y-10">
              ...
              <p className="text-[11px] font-black text-secondary uppercase tracking-[0.4em]">
                {tr('Handshaking with global SRE knowledge mesh...')}
              </p>
            </div>
          </div>
        )}
        ...
        {answer && (
          <div className="bg-white rounded-[4rem] border border-[#E5E5EA] p-16 shadow-soft animate-fade-in-up relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-16 opacity-5 pointer-events-none group-hover:scale-110 transition-transform">
              <Sparkles className="text-[150px] text-primary" />
            </div>
            <div className="flex items-center gap-6 mb-12 border-b border-[#F5F5F7] pb-10 relative z-10">
              <div className="p-4 bg-indigo-600 text-white rounded-[1.5rem] shadow-2xl shadow-indigo-600/30">
                <BadgeCheck className="text-[32px]" />
              </div>
              <div>
                <h3 className="font-black text-text-main text-xl uppercase tracking-tighter leading-none">
                  {tr('AI Neural Insight')}
                </h3>
                <p className="text-indigo-600 text-[11px] font-black uppercase tracking-widest mt-2">
                  {tr('Grounded Domain Synthesis Active')}
                </p>
              </div>
            </div>
            ...
            {referenceChunks.length > 0 && (
              <div className="mt-16 pt-12 border-t border-[#F5F5F7] relative z-10">
                <p className="text-[11px] font-black text-secondary uppercase tracking-[0.4em] mb-10">
                  {tr('Expert Reference Nodes')}
                </p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {referenceChunks.map((chunk) => (
                    <a
                      ...
                      className="flex items-center justify-between p-8 bg-[#F5F5F7]/50 rounded-[2.5rem] hover:bg-white hover:shadow-soft border border-transparent hover:border-[#E5E5EA] transition-all group"
                    >
                      <div className="flex items-center gap-6 overflow-hidden">
                        <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center border border-[#E5E5EA] text-primary group-hover:bg-primary group-hover:text-white transition-all shadow-sm">
                          <Link className="text-[20px]" />
                        </div>
                        <span className="text-sm font-black text-text-main truncate tracking-tight">
                          {chunk.title}
                        </span>
                      </div>
                      <ArrowUpRight className="text-[#C1C1C1] group-hover:text-primary text-[24px] group-hover:translate-x-1 transition-all" />
                    </a>
                  ))}
                </div>
              </div>

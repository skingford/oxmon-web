  ...
  return (
    <div className="space-y-10 animate-fade-in h-full flex flex-col pb-16">
      <div className="space-y-3">
        <h2 className="text-5xl font-black text-text-main tracking-tighter uppercase">{tr('Config Forge')}</h2>
        <p className="text-secondary text-base font-medium">{tr('Neural synthesis for infrastructure-as-code and mission-critical server configurations.')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {TEMPLATES.map((t) => (
              ...
              >
                  ...
                  <div>
                    <p className="text-[11px] font-black text-secondary uppercase tracking-[0.3em] group-hover:text-primary mb-3 transition-colors">{tr(t.title)}</p>
                    <p className="text-[11px] text-secondary font-bold line-clamp-2 leading-relaxed opacity-60 tracking-tight">{tr(t.prompt)}</p>
                  </div>
              </button>
          ))}
      </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-12 min-h-0">
        <div className="w-full lg:w-[480px] bg-white rounded-[4rem] border border-[#E5E5EA] p-12 shadow-soft flex flex-col gap-12 shrink-0">
            <div>
                <h3 className="text-[11px] font-black text-secondary uppercase tracking-[0.5em] mb-8">{tr('Neural Logic Input')}</h3>
                <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    placeholder={tr('Describe your target infrastructure object in detail...')}
            ...
            <div className="space-y-6">
                <div className="flex items-center justify-between p-10 bg-indigo-50/40 rounded-[3rem] border border-indigo-100 shadow-sm group">
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <span className="text-[11px] font-black text-indigo-950 uppercase tracking-[0.3em]">{tr('Neural Hardening')}</span>
                            <p className="text-[9px] font-black text-indigo-500 uppercase mt-1 tracking-widest">{tr('SRE Audit Active')}</p>
                        </div>
                    </div>
                    <button
                ...
                >
                    ...
                    {isGenerating ? tr('Synthesizing...') : tr('Forge Infrastructure')}
                </button>
            </div>
                ...
                <div className="flex-1 p-16 overflow-y-auto custom-scrollbar font-mono text-[15px] relative z-10">
                    {isGenerating ? (
                        <div className="h-full flex flex-col items-center justify-center space-y-12">
                            ...
                            <p className="text-indigo-200 font-mono text-xs uppercase tracking-[0.6em] animate-pulse">{tr('Initializing Synthesis')}</p>
                        </div>
                    ) : generatedCode ? (
                        ...
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-10 grayscale space-y-12">
                            ...
                            <p className="text-[14px] font-black uppercase tracking-[0.6em]">{tr('Await Neural Handshake')}</p>
                        </div>
                    )}
                </div>
            </div>
            ...
            {hardeningReport && (
                <div className="bg-white rounded-[3.5rem] p-12 border border-[#E5E5EA] shadow-soft animate-fade-in-up">
                    <div className="flex items-center gap-5 mb-8">
                        ...
                        <h4 className="text-[12px] font-black text-text-main uppercase tracking-[0.5em]">{tr('Audit Disclosure Summary')}</h4>
                    </div>

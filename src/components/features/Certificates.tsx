...
const Certificates: React.FC<CertificatesProps> = ({ certificates, onAddCertificate, onDeleteCertificate, onRenewCertificate, onShowToast }) => {
  ...
  const handleAnalyzeTrust = async (cert: Certificate) => {
    ...
    try {
        const result = await analyzeCertificateTrust(cert.domain, cert.issuer, cert.daysRemaining);
        setAnalysisResult(result);
    } catch (err) { onShowToast(tr('Neural audit handshake failed.'), 'error'); } finally { setIsAnalyzing(false); }
  };
    ...
    <div className="space-y-12 animate-fade-in pb-20">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8">
        <div className="space-y-3">
          <h2 className="text-5xl font-black text-text-main tracking-tighter uppercase">{tr('Trust Perimeter')}</h2>
          <p className="text-secondary text-base font-medium">{tr('Global lifecycle management for TLS/SSL credentials and distributed trust chains.')}</p>
        </div>
        <div className="flex items-center gap-4">
           <div className="relative group">
            ...
            <input type="text" placeholder={tr('Filter domains...')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-12 pr-6 py-3.5 bg-white border border-border rounded-2xl text-sm w-72 shadow-soft focus:ring-8 focus:ring-primary/5 transition-all outline-none font-bold" />
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
            { label: tr('Domains Protected'), val: certificates.length, color: 'text-text-main', icon: 'verified_user' },
            { label: tr('Secure Gradient'), val: certificates.filter(c => c.status === 'Valid').length, color: 'text-success', icon: 'shield_with_heart' },
            { label: tr('Expiring Soon'), val: certificates.filter(c => c.status === 'Expiring').length, color: 'text-warning', icon: 'notification_important' },
            { label: tr('TTL Overdue'), val: certificates.filter(c => c.status === 'Expired').length, color: 'text-danger', icon: 'error' }
        ].map(stat => (
      ...
      <div className="bg-white rounded-[3.5rem] shadow-soft border border-border overflow-hidden">
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#FBFBFD] border-b border-border">
            <tr>
              <th className="py-7 px-12 text-[11px] font-black text-secondary uppercase tracking-[0.4em]">{tr('Domain Endpoint')}</th>
              <th className="py-7 px-12 text-[11px] font-black text-secondary uppercase tracking-[0.4em]">{tr('Trust Integrity')}</th>
              <th className="py-7 px-12 text-[11px] font-black text-secondary uppercase tracking-[0.4em]">{tr('CA Entity')}</th>
              <th className="py-7 px-12 text-[11px] font-black text-secondary uppercase tracking-[0.4em]">{tr('TTL Remaining')}</th>
              ...
              <tr key={cert.id} className="hover:bg-gray-50/50 transition-all group cursor-pointer" onClick={() => setDetailCert(cert)}>
                ...
                <td className="py-8 px-12">
                    <div className="flex flex-col">
                        <span className={`text-lg font-black tracking-tight ${cert.daysRemaining < 15 ? 'text-danger' : 'text-text-main'}`}>{cert.daysRemaining} {tr('Days')}</span>
                ...
                <tr>
                    <td colSpan={5} className="py-32 text-center opacity-30">
                        <div className="flex flex-col items-center gap-6">
                            ...
                            <p className="text-[12px] font-black uppercase tracking-[0.5em]">{tr('No matching trust assets identified.')}</p>
                        </div>
                    </td>
                </tr>
            )}
            ...
            <div className="relative w-full max-w-2xl bg-white h-full shadow-3xl border-l border-border p-16 animate-slide-in-right flex flex-col overflow-hidden">
                <div className="flex justify-between items-start mb-16">
                    <div className="space-y-2">
                        <h3 className="text-3xl font-black text-text-main tracking-tighter uppercase leading-none">{tr('Trust Analysis')}</h3>
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-14">
                    <div className="p-12 bg-gradient-to-br from-indigo-50/50 to-white rounded-[3.5rem] border border-indigo-100 shadow-soft relative overflow-hidden group">
                        ...
                        <p className="text-[11px] font-black text-primary uppercase tracking-[0.5em] mb-8">{tr('Asset Telemetry')}</p>
                        <div className="grid grid-cols-2 gap-10">
                            <div className="space-y-2">
                                <p className="text-[10px] text-secondary uppercase font-black tracking-widest opacity-40">{tr('Status')}</p>
                                <p className="text-xl font-black text-indigo-950 uppercase tracking-tight">{tr(detailCert.status)}</p>
                            </div>
                            <div className="space-y-2 text-right">
                                <p className="text-[10px] text-secondary uppercase font-black tracking-widest opacity-40">{tr('TTL')}</p>
                                <p className="text-xl font-black text-indigo-950 tracking-tight">{detailCert.daysRemaining} {tr('Days')}</p>
                            </div>
                            <div className="space-y-2">
                                <p className="text-[10px] text-secondary uppercase font-black tracking-widest opacity-40">{tr('Authority')}</p>
                                <p className="text-[13px] font-black text-indigo-950 uppercase tracking-widest">{detailCert.issuer}</p>
                            </div>
                            <div className="space-y-2 text-right">
                                <p className="text-[10px] text-secondary uppercase font-black tracking-widest opacity-40">{tr('Last Sync')}</p>
                    ...
                    <div className="space-y-8">
                        <div className="flex items-center justify-between">
                            <h5 className="text-[11px] uppercase font-black text-secondary tracking-[0.5em]">{tr('Neural Trust Audit')}</h5>
                            <button onClick={() => handleAnalyzeTrust(detailCert)} disabled={isAnalyzing} className="text-[11px] font-black text-primary uppercase tracking-widest flex items-center gap-3 disabled:opacity-50 group/audit">
                                ...
                                {isAnalyzing ? tr('Synchronizing...') : tr('Refresh Neural Audit')}
                            </button>
                        </div>
                        ...
                        {analysisResult ? (
                            ...
                            <div className="p-24 border-4 border-dashed border-gray-100 rounded-[3.5rem] text-center space-y-8 group hover:border-primary/20 transition-all">
                                ...
                                <p className="text-[11px] font-black text-secondary uppercase tracking-[0.5em] leading-[2.5]">{tr('Establish neural link for high-fidelity trust chain vulnerability assessment')}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="pt-12 border-t border-border flex gap-5">
                    <button onClick={() => { onRenewCertificate(detailCert.id); setDetailCert(null); }} className="flex-1 py-6 bg-primary text-white rounded-[2rem] font-black text-[11px] uppercase tracking-[0.4em] shadow-3xl shadow-primary/40 hover:bg-primary-hover active:scale-95 transition-all">{tr('Renew Trust Asset')}</button>
                    <button onClick={() => { onDeleteCertificate(detailCert.id); setDetailCert(null); }} className="px-10 py-6 border-2 border-border rounded-[2rem] font-black text-[11px] uppercase tracking-[0.4em] text-danger hover:bg-red-50 hover:border-red-100 transition-all active:scale-95">{tr('Decommission')}</button>
                </div>
            </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-8">
          ...
          <div className="relative bg-white rounded-[4rem] shadow-3xl w-full max-w-xl p-16 animate-fade-in-up border border-white/20 ring-1 ring-black/5">
            <div className="flex items-center gap-6 mb-12">
                ...
                <h3 className="text-3xl font-black text-text-main tracking-tighter uppercase leading-none">{tr('Provision Monitor')}</h3>
            </div>
            <form onSubmit={handleSubmit} className="space-y-10">
              <div className="space-y-3">
                  <label className="block text-[11px] font-black text-secondary uppercase tracking-[0.4em] ml-1 opacity-50">{tr('FQDN / Endpoint Identifier')}</label>
                  <input required type="text" value={newCert.domain} onChange={(e) => setNewCert({...newCert, domain: e.target.value})} placeholder={tr('e.g. sentinel.oxmon.io')} className="w-full px-8 py-5 rounded-[2rem] bg-gray-50 border border-border outline-none focus:ring-8 focus:ring-primary/5 focus:bg-white transition-all text-base font-bold placeholder:text-gray-300" />
              </div>
              <div className="space-y-3">
                  <label className="block text-[11px] font-black text-secondary uppercase tracking-[0.4em] ml-1 opacity-50">{tr('CA Issuing Authority')}</label>
                  <input required type="text" value={newCert.issuer} onChange={(e) => setNewCert({...newCert, issuer: e.target.value})} placeholder={tr('e.g. DigiCert Global G2')} className="w-full px-8 py-5 rounded-[2rem] bg-gray-50 border border-border outline-none focus:ring-8 focus:ring-primary/5 focus:bg-white transition-all text-base font-bold placeholder:text-gray-300" />
              </div>
              <div className="space-y-3">
                  <label className="block text-[11px] font-black text-secondary uppercase tracking-[0.4em] ml-1 opacity-50">{tr('Handshake Expiry')}</label>
              ...
              <div className="flex justify-end gap-5 pt-8">
                  <button type="button" onClick={() => setIsModalOpen(false)} className="px-8 py-5 rounded-2xl text-[11px] font-black uppercase tracking-[0.4em] text-secondary hover:text-text-main transition-colors">{tr('Discard')}</button>
                  <button type="submit" className="px-12 py-5 rounded-[2rem] bg-primary text-white font-black text-[11px] uppercase tracking-[0.4em] shadow-3xl shadow-primary/30 hover:bg-primary-hover active:scale-95 transition-all">{tr('Enable Monitor')}</button>
              </div>
            </form>
          </div>
        </div>

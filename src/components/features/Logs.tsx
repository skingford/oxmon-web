    ...
    <div className="space-y-10 animate-fade-in h-full flex flex-col pb-16">
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div className="space-y-3">
          <h2 className="text-5xl font-black text-text-main tracking-tighter uppercase">{tr('Audit Stream')}</h2>
          <p className="text-secondary text-base font-medium">{tr('Distributed cluster observability, neural security logging, and principal access tracking.')}</p>
        </div>
        <div className="flex flex-wrap gap-4 items-center">
          <div className="relative group">
            ...
            <input type="text" placeholder={tr('Grep telemetry stream...')} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-12 pr-6 py-3.5 bg-white border border-border rounded-2xl text-sm w-72 shadow-soft focus:ring-8 focus:ring-primary/5 transition-all outline-none font-bold" />
          </div>
      ...
      <div className="bg-white p-12 rounded-[3.5rem] border border-border shadow-soft relative overflow-hidden group">
        ...
        <div className="flex items-center justify-between mb-10 relative z-10">
            <span className="text-[11px] font-black text-secondary uppercase tracking-[0.4em]">{tr('Cluster Incident Gradient')}</span>
            <div className="flex items-center gap-8">
                <div className="flex items-center gap-3"><span className="w-2.5 h-2.5 rounded-full bg-danger shadow-lg shadow-danger/30"></span><span className="text-[10px] font-black uppercase tracking-widest text-secondary opacity-60">{tr('Critical Payload')}</span></div>
                <div className="flex items-center gap-3"><span className="w-2.5 h-2.5 rounded-full bg-warning shadow-lg shadow-warning/30"></span><span className="text-[10px] font-black uppercase tracking-widest text-secondary opacity-60">{tr('Anomalous Drift')}</span></div>
            </div>
        </div>
      ...
      {logAnalysis && (
        <div className="bg-[#0F172A] text-white rounded-[3.5rem] p-16 shadow-3xl animate-fade-in-up border border-white/5 relative overflow-hidden shrink-0 group">
            ...
            <div className="flex items-center gap-6 mb-12 relative z-10">
                ...
                <div>
                    <h3 className="font-black text-lg tracking-tighter uppercase text-indigo-300 leading-none">{tr('Neural Pattern Detection')}</h3>
                    <p className="text-indigo-500 text-[11px] font-black uppercase mt-3 tracking-widest">{tr('Global Cross-Log Correlation Logic Active')}</p>
                </div>
            </div>
      ...
      <div className="flex-1 bg-white rounded-[3.5rem] border border-border shadow-soft overflow-hidden flex flex-col font-mono text-[12px] min-h-0 ring-1 ring-black/5">
        <div className="bg-[#FBFBFD] px-12 py-7 border-b border-border grid grid-cols-12 gap-8 text-secondary font-black uppercase tracking-[0.2em] sticky top-0 z-10 shadow-sm">
            <div className="col-span-3 lg:col-span-2">{tr('Telemetry Index')}</div>
            <div className="col-span-2 lg:col-span-1">{tr('State')}</div>
            <div className="col-span-2 lg:col-span-1 text-center">{tr('Identity')}</div>
            <div className="col-span-5 lg:col-span-8">{tr('High-Fidelity Payload')}</div>
        </div>
        <div ref={scrollRef} className="flex-1 overflow-y-auto divide-y divide-border custom-scrollbar">
            {filteredLogs.length > 0 ? filteredLogs.map((log) => (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-40 opacity-20 text-center space-y-8 grayscale">
                    ...
                    <p className="text-[14px] font-black uppercase tracking-[0.6em] leading-loose">{tr('No Telemetry Drift Identified in Selected Buffer')}</p>
                </div>
            )}
        </div>
      </div>

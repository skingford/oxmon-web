...
type CertificateExpirationNotificationSettingsPageProps = {
  locale: 'en' | 'zh'
}
...
const USER_AVATAR_URL =
  'https://lh3.googleusercontent.com/aida-public/AB6AXuD9aG6HN6jeYQLEc4IZNiKWyNgRdLCT2R0QjZxAjdozS0GI9IYtsy67iM1lXH3URvB4JKFoFfKN0M9A7_MjxJHY6Axr9b2p3CCcGTB_BlYaj0A2ymdFHrAdkdcgxPLDx3dNwtieyfiimHa1CmA50ZFbd2u-jSiNUn2ZPiFQqFY_gewB8fbJkCEATrXDHPlejM34E3WN-tGNxMvEjDNehbq_A7Mx1VixtVUDe8TsRKPPHYiMctxgtC9r_jQU-q2PxhLi54ucgtwqGeU'
...
export default function CertificateExpirationNotificationSettingsPage({ locale }: CertificateExpirationNotificationSettingsPageProps) {
  const isZh = locale === 'zh'

  return (
    <div className="relative flex min-h-screen w-full flex-col overflow-x-hidden bg-[#F5F5F7] font-sans text-[#1D1D1F] antialiased">
      <div className="flex h-full grow flex-col">
        <header className="sticky top-0 z-50 flex items-center justify-between whitespace-nowrap border-b border-[#D2D2D7]/50 bg-white/80 px-6 py-3 backdrop-blur-md">
          <div className="flex items-center gap-4">
            <div className="flex size-8 items-center justify-center rounded-lg bg-[#0071E3] text-white">
              <span className="material-symbols-outlined text-[20px]">monitoring</span>
            </div>
            <h2 className="text-lg font-semibold tracking-tight text-[#1D1D1F]">Oxmon</h2>
          </div>

          <div className="flex flex-1 items-center justify-end gap-6">
            <button type="button" className="text-sm font-medium text-[#86868B] transition-colors hover:text-[#1D1D1F]">
              {isZh ? '帮助' : 'Help'}
            </button>
            <div className="h-4 w-px bg-[#D2D2D7]" />
            <div
              className="size-8 rounded-full bg-cover bg-center bg-no-repeat shadow-sm ring-2 ring-white"
        ...
        <div className="flex flex-1">
          <nav className="hidden w-64 flex-col border-r border-[#D2D2D7] bg-white px-4 pt-6 pb-10 lg:flex">
            <div className="flex flex-col gap-1">
              <div className="mb-4 px-3 text-xs font-semibold tracking-wider text-[#86868B] uppercase">
                {isZh ? '基础设施' : 'Infrastructure'}
              </div>
              <SidebarItem icon="dashboard" label={isZh ? '仪表盘' : 'Dashboard'} />
              <SidebarItem icon="show_chart" label={isZh ? '监控' : 'Monitors'} />
              <SidebarItem icon="verified_user" label={isZh ? '证书' : 'Certificates'} active />
              <SidebarItem icon="notifications" label={isZh ? '告警' : 'Alerts'} />
            </div>
            ...
            <div className="mt-8 flex flex-col gap-1">
              ...
              <SidebarItem icon="group" label={isZh ? '团队' : 'Team'} />
            </div>
          </nav>

          <main className="mx-auto w-full max-w-5xl flex-1 px-6 py-8 lg:px-12">
            <div className="mb-6 flex items-center gap-2 text-sm text-[#86868B]">
              <Link href={`/${locale}/settings`} className="transition-colors hover:text-[#0071E3]">{isZh ? '设置' : 'Settings'}</Link>
              <span className="text-[#D2D2D7]">/</span>
              <Link href={`/${locale}/alert-rules-configuration-tab`} className="transition-colors hover:text-[#0071E3]">{isZh ? '通知' : 'Notifications'}</Link>
              <span className="text-[#D2D2D7]">/</span>
              <span className="font-medium text-[#1D1D1F]">{isZh ? '证书过期' : 'Certificate Expiration'}</span>
            </div>

            <div className="mb-10">
              <h1 className="mb-2 text-3xl font-bold tracking-tight text-[#1D1D1F]">{isZh ? '证书过期' : 'Certificate Expiration'}</h1>

...
type MetricsQueryToolbarProps = {
  ...
  onAgentSearchKeywordChange: (value: string) => void;
...
export function MetricsQueryToolbar({
  ...
  const normalizedAgentSearch = agentSearchKeyword.trim().toLowerCase();
  ...
  const handleSelectAgent = (agentId: string) => {
    ...
    setRecentAgents((prev) => {
      const next = [agentId, ...prev.filter((id) => id !== agentId)].slice(
        0,
        20,
      );
      try {
  ...
  const filteredAgents = useMemo(() => {
    ...
    const scored = orderedAgents
      ...
      .filter(
        (item): item is { agentId: string; score: number; index: number } =>
          item !== null,
      )
      .sort((a, b) => {
        ...
        return (
          a.score - b.score ||
          a.index - b.index ||
          (recentA ?? Number.MAX_SAFE_INTEGER) -
            (recentB ?? Number.MAX_SAFE_INTEGER) ||
  ...
  const sourceOptions = useMemo<SearchableComboboxOption[]>(
    () =>
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
      <div className="space-y-2">
        ...
        <SearchableCombobox
          ...
          searchValue={agentSearchKeyword}
          onSearchValueChange={onAgentSearchKeywordChange}
          clearSearchOnClose
          clearSearchOnSelect
          disableClientFilter
        />
      ...
      <div className="space-y-2">
        ...
        <Select
          ...
          onValueChange={onSelectedMetricChange}
          disabled={fetchingOptions}
        >
      ...
      <div className="space-y-2">
        <div className="flex h-5 items-center text-sm text-muted-foreground">
          {texts.queryModeLabel}
        </div>
      ...
      <div className="space-y-2">
        <div className="flex h-5 items-center text-sm text-muted-foreground">
          {texts.timeRangeLabel}
        </div>
        <Select
          value={timeRange}
          onValueChange={(value) =>
            onTimeRangeChange(value as MetricsTimeRange)
          }
        >
          <SelectTrigger className="h-10">
            <SelectValue />
      ...
      <div className="space-y-2 md:col-span-2">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <Button
            ...
            onClick={onRefresh}
            disabled={!hasQueryCondition || querying}
          >
            {querying ? (
          ...
          <Button
            ...
            size="icon"
            className="h-10 w-10"
            onClick={onCopyQueryLink}
            title={texts.copyQueryLinkTitle}
          >

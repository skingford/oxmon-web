      ...
      <div className="relative flex items-center overflow-hidden rounded-xl border-2 border-[#0673e0] bg-blue-50/30 p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04),0_1px_2px_rgba(0,0,0,0.02)]">
        ...
        <div className="min-w-0 flex-1">
          <div className="mb-1 flex items-center gap-2">
            <span className="text-xs font-bold tracking-wider text-[#0673e0] uppercase">
              {node.type}
            </span>
            <CheckCircle2 className="text-[16px] text-[#34c759]" />
          </div>
          <h3 className="truncate text-lg font-bold text-[#1d1d1f]">
            {node.title}
          </h3>
        ...
        <div className="ml-4">
          <Button
            type="button"
            className="text-sm font-medium text-[#0673e0] transition-colors hover:text-blue-700"
          >
            Details
          </Button>
        </div>
...
export default function CertificateChainVisualPath({
  locale,
}: CertificateChainVisualPathProps) {
      ...
      <header className="w-full border-b border-gray-200 bg-white px-6 py-4">
        <div className="mx-auto w-full max-w-5xl">
          <nav className="flex items-center text-sm font-medium text-[#86868b]">
            <Link
              href={`/${locale}/domains`}
              className="transition-colors hover:text-[#0673e0]"
            >
              {isZh ? '证书' : 'domains'}
            </Link>
            <span className="mx-2 text-gray-300">/</span>
            <Link
              href={`/${locale}/certificate-detail-view-example-com`}
              className="transition-colors hover:text-[#0673e0]"
            >
              {isZh ? '资产清单' : 'Inventory'}
            </Link>
          ...
          <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
            <div>
              <h1 className="text-3xl font-bold tracking-tight text-[#1d1d1f]">
                {isZh ? '证书链' : 'Certificate Chain'}
              </h1>
              <p className="mt-1 text-lg text-[#86868b]">
                {isZh
                  ? '*.example.com 的可视化路径'
                  : 'Visual path for *.example.com'}
              </p>
            </div>
          ...
          <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            <article className="flex flex-col gap-1 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-medium text-[#86868b]">
                {isZh ? '链路状态' : 'Chain Status'}
              </p>
              <div className="flex items-center gap-2">
                <span className="leading-none text-2xl font-bold text-[#1d1d1f]">
                  {isZh ? '有效' : 'Valid'}
                </span>
                <BadgeCheck className="text-[24px] text-[#34c759]" />
              </div>
              <p className="mt-1 text-xs font-medium text-green-600">
                + {isZh ? '已通过信任库验证' : 'Verified against trust store'}
              </p>
            </article>

            <article className="flex flex-col gap-1 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-medium text-[#86868b]">
                {isZh ? '链路深度' : 'Chain Depth'}
              </p>
            ...
            <article className="flex flex-col gap-1 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
              <p className="text-sm font-medium text-[#86868b]">
                {isZh ? '签名算法' : 'Signature Algorithm'}
              </p>
              <span className="text-2xl font-bold text-[#1d1d1f]">SHA-256</span>
              <p className="mt-1 text-xs text-[#86868b]">
                {isZh ? '使用 RSA 加密' : 'with RSA Encryption'}
              </p>
            </article>
          </section>
              ...
              <div className="flex gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <h4 className="text-base font-bold text-[#1d1d1f]">
                    {isZh ? '证书链验证成功' : 'Chain Validation Successful'}
                  </h4>

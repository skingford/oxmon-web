...
type CertificateDetailViewExampleComProps = {
  locale: 'en' | 'zh'
}
...
export default function CertificateDetailViewExampleCom({
  locale,
}: CertificateDetailViewExampleComProps) {
  return (
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between whitespace-nowrap border-b border-[#e5e7eb] bg-white/80 px-6 py-3 backdrop-blur-md">
        <div className="flex items-center gap-8">
          <div className="flex items-center gap-3 text-[#0073e6]">
            <div className="flex size-8 items-center justify-center rounded-lg bg-[#0073e6] text-white">
              <Shield className="text-[20px]" />
            </div>
            <h2 className="text-xl font-bold leading-tight tracking-tight text-[#111827]">
              Oxmon
            </h2>
          </div>
        ...
        <div className="flex items-center gap-6">
          <nav className="hidden items-center gap-6 lg:flex">
            <Link
              href="#"
              className="text-sm font-medium leading-normal text-[#6b7280] transition-colors hover:text-[#0073e6]"
            >
              Dashboard
            </Link>
            <Link
              href="#"
              className="rounded-md bg-gray-100 px-3 py-1.5 text-sm font-medium leading-normal text-[#111827]"
            >
              domains
            </Link>
            <Link
          ...
          <div className="flex items-center gap-4 border-l border-gray-200 pl-6">
            <Button
              type="button"
              className="relative text-[#6b7280] transition-colors hover:text-[#111827]"
            >
              <Bell />
            ...
            <div
              className="size-9 cursor-pointer rounded-full bg-cover bg-center bg-no-repeat ring-2 ring-gray-100"
      ...
      <main className="mx-auto w-full max-w-[1200px] flex-1 p-6 md:p-8 lg:px-12">
        <nav className="mb-6 flex items-center text-sm text-[#6b7280]">
          <Link
            ...
            className="flex items-center gap-1 transition-colors hover:text-[#0073e6]"
          >
            <House className="text-[18px]" />
          </Link>
          {BREADCRUMBS.map((item) => (
            <div key={item.label} className="flex items-center">
              ...
              <Link
                href={item.href}
                className="transition-colors hover:text-[#0073e6]"
              >
                {item.label}
              </Link>
            </div>
        ...
        <div className="mb-8 flex flex-col justify-between gap-4 md:flex-row md:items-start">
          <div>
            ...
            <p className="text-sm text-[#6b7280] md:text-base">
              SSL/TLS Certificate details and verification status.
            </p>
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <Download className="mr-2 text-[18px]" />
              Download
            </Button>
        ...
        <section className="mb-8 rounded-2xl border border-gray-100 bg-white p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.05),0_1px_2px_-1px_rgba(0,0,0,0.05)] md:p-8">
          <div className="mb-6 flex items-center justify-between border-b border-gray-100 pb-4">
            <h3 className="flex items-center gap-2 text-lg font-semibold text-[#111827]">
              <ShieldCheck className="text-[#0073e6]" />
          ...
          <div className="grid grid-cols-1 gap-x-12 gap-y-8 md:grid-cols-2 lg:grid-cols-3">
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wider text-[#6b7280]">
                {OVERVIEW_ROWS[0].label}
              </span>
              <div className="group flex cursor-pointer items-center gap-2">
                <span className="text-base font-semibold text-[#111827]">
                  {OVERVIEW_ROWS[0].value}
                </span>
                <Copy className="text-[16px] text-gray-400 opacity-0 transition-opacity group-hover:opacity-100" />
              </div>
            </div>

            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wider text-[#6b7280]">
                {OVERVIEW_ROWS[1].label}
              </span>
              <div className="flex items-center gap-2">
                <div className="flex size-5 items-center justify-center rounded-full bg-blue-100 text-[10px] font-bold text-blue-600">
                  L
                </div>
                <span className="text-base font-medium text-[#111827]">
                  {OVERVIEW_ROWS[1].value}
                </span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wider text-[#6b7280]">
                {OVERVIEW_ROWS[2].label}
              </span>
              <div className="flex items-center gap-3">
                <span className="text-2xl font-bold tracking-tight text-[#111827]">
                  {OVERVIEW_ROWS[2].value}
                </span>
            ...
            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wider text-[#6b7280]">
                {OVERVIEW_ROWS[3].label}
              </span>
              <span className="font-mono text-base text-[#111827]">
                {OVERVIEW_ROWS[3].value}
              </span>
            </div>

            <div className="flex flex-col gap-1">
              <span className="text-xs font-medium uppercase tracking-wider text-[#6b7280]">
                {OVERVIEW_ROWS[4].label}
              </span>
              <span className="font-mono text-base text-[#111827]">
                {OVERVIEW_ROWS[4].value}
              </span>
            </div>
        ...
        <section className="mb-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
          <div className="flex h-full flex-col rounded-2xl border border-gray-100 bg-white shadow-[0_1px_3px_0_rgba(0,0,0,0.05),0_1px_2px_-1px_rgba(0,0,0,0.05)]">
                ...
                <TableBody className="divide-y divide-gray-50 bg-white">
                  {SAN_ROWS.map((row) => (
                    <TableRow key={row.name}>
                      <TableCell className="whitespace-nowrap px-4 py-3 font-mono text-sm text-[#111827]">
                        {row.name}
                      </TableCell>
                      <TableCell className="whitespace-nowrap px-4 py-3 text-right text-sm text-[#6b7280]">
                        {row.type}
                      </TableCell>
                    </TableRow>
                  ))}
            ...
            <div className="mt-auto rounded-b-2xl border-t border-gray-100 bg-gray-50 px-6 py-3">
              <Button
                type="button"
                className="flex items-center gap-1 text-sm font-medium text-[#0073e6] transition-colors hover:text-[#0073e6]/80"
              >
                View all 3 SANs
          ...
          <div className="flex flex-col items-start gap-6 p-6 md:flex-row md:items-center md:p-8">
                ...
                {CHAIN_ROWS.map((row) => {
                  ...
                  return (
                    <div key={row.title} className="flex items-center gap-4">
                      ...
                      >
                        <RowIcon
                          className={`text-[20px] ${row.leaf ? 'text-green-600' : 'text-gray-500'}`}
                        />
                      </div>

                      <div className="flex flex-col">
                        <span
                          className={`text-sm ${row.leaf ? 'font-bold' : 'font-semibold'} text-[#111827]`}
                        >
                          {row.title}
                        </span>
                        <span className="text-xs text-[#6b7280]">
            ...
            <div className="flex w-full flex-1 items-start gap-4 rounded-xl border border-green-100 bg-green-50/50 p-6">
              <div className="rounded-full bg-green-100 p-2 text-green-600">
                <BadgeCheck className="text-[24px]" />
              </div>
              <div>
                <h4 className="mb-1 text-base font-bold text-green-800">
                  Chain is complete and trusted
                </h4>
                <p className="leading-relaxed text-green-700/80">
                  The certificate chain is correctly configured. The root
                  certificate is trusted by major browsers and operating systems
                  (Mozilla, Google, Apple, Microsoft).
                </p>
              </div>
            </div>
          </div>

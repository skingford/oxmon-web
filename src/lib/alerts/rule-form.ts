...
export const ALERT_RULE_TYPE_OPTIONS = SUPPORTED_ALERT_RULE_TYPES.map((value) => ({
  value,
...
export function isSupportedAlertRuleType(value: string): value is SupportedAlertRuleType {
...
export function getLegacyAlertRuleTypeAlias(value: string): SupportedAlertRuleType | null {
  const normalized = value.trim().toLowerCase()
  return LEGACY_ALERT_RULE_TYPE_ALIASES[normalized] || null
}

export function getAlertRuleTypeLabelInfo(ruleType: string): AlertRuleTypeLabelInfo {
  ...
  if (getLegacyAlertRuleTypeAlias(ruleType)) {
    return {
...
export function normalizeAlertRulePayload(form: CreateAlertRuleRequest): CreateAlertRuleRequest {
  return {
    ...form,

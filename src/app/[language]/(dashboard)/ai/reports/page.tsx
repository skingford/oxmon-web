...
function riskVariant(risk: string): "default" | "destructive" | "secondary" | "outline" {
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">{t("reports.colActions")}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {loading ? (
                ...
                sortedItems.map((item) => (
                  <TableRow key={item.id}>
                    ...
                    <TableCell className="max-w-[220px] truncate">{item.ai_model}</TableCell>
                    <TableCell>
                      <Badge variant={riskVariant(item.risk_level)}>{item.risk_level}</Badge>
                    </TableCell>
                    <TableCell>{item.total_agents}</TableCell>
                    <TableCell>{item.notified ? t("reports.notifiedYes") : t("reports.notifiedNo")}</TableCell>
